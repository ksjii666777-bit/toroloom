/**
 * ============================================================================
 * Toroloom — riskEngine.resetForTesting() Cross-File Isolation (MongoDB)
 * ============================================================================
 *
 * Validates that resetForTesting() properly isolates the riskEngine singleton
 * across test file boundaries when using a MongoDB storage backend.
 *
 * These tests simulate the scenario that caused cross-file contamination:
 *   File A configures MongoDB storage → writes data → afterAll resets
 *   File B configures MongoDB storage → should see clean state
 *
...
const USER_A = 'reset_mongo_user_a';
const USER_B = 'reset_mongo_user_b';
...
describe('riskEngine.resetForTesting() — MongoDB Cross-File Isolation', () => {
  ...
  it('should isolate profiles across simulated file boundaries after reset', async () => {
    ...
    // Load USER_A from DB — should still exist since reset clears the
    // cache, not the DB
