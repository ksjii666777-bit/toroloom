/**
 * ============================================================================
 * Toroloom Broker State Persistence — MongoDB Integration
 * ============================================================================
 *
 * Validates broker failover state and dedup cache persistence against
 * MongoDB. These tests go beyond the basic round-trip test in
 * storageMongoDB.int.test.ts by verifying:
 *
 ...
 *
 * The broker state stored as a single document in the broker_state
 * collection, with _id = 'broker_state'. The `replaceOne({ _id: 'broker_state' }, ...)`
 * with upsert: true always replaces the entire document.
 *
 * Environment:
 ...
 *
 * Run:
 *   npx vitest run --reporter=verbose src/__tests__/brokerState.mongodb.int.test.ts
 *
 * Skip:
...
describe.skipIf(!hasTestMongo)('Broker State Persistence — MongoDB', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.warn(
        `⚠ MongoDB not available (${err.message}) — skipping Broker State + Mongo integration tests`,
      );
      available = false;
    }
  }, 30_000);
  ...
  it('should completely replace broker state on save (replaceOne overwrites entire document)', async () => {
    if (!available) return;
  ...
  it('should survive a simulated crash/restart via independent storage instance', async () => {
    ...
    const storage1 = new MongoDBStorage(MONGODB_URI, MONGODB_DB_NAME);
    ...
    const storage2 = new MongoDBStorage(MONGODB_URI, MONGODB_DB_NAME);
