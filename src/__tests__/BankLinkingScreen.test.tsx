...
describe('BankLinkingScreen', () => {
  ...
  it('renders the header with title', () => {
    const { getByText } = render(<BankLinkingScreen {...baseProps as any} />);
  ...
  it('shows IFSC input with correct placeholder', () => {
    const { getByPlaceholderText } = render(<BankLinkingScreen {...baseProps as any} />);
    expect(getByPlaceholderText('HDFC0001234')).toBeDefined();
  });

  it('shows Verify IFSC button', () => {
    const { getAllByText } = render(<BankLinkingScreen {...baseProps as any} />);
  ...
  it('filters non-alphanumeric characters and uppercases IFSC input', () => {
    const { getByPlaceholderText, getByText } = render(<BankLinkingScreen {...baseProps as any} />);
  ...
  it('advances to account step (shows Account Number input) after successful IFSC verification', async () => {
    vi.useFakeTimers();
    const { getByPlaceholderText, getAllByText, getByText } = render(<BankLinkingScreen {...baseProps as any} />);
    const input = getByPlaceholderText('HDFC0001234');
    act(() => { fireEvent.changeText(input, 'HDFC0001234'); });
  ...
  it('renders without crashing', () => {
    const { toJSON } = render(<BankLinkingScreen {...baseProps as any} />);
    expect(toJSON()).toBeTruthy();
  });

  it('shows the back button and navigates back on press', () => {
    const { getByTestId } = render(<BankLinkingScreen {...baseProps as any} />);
  ...
  it('allows navigating to manage step when manage button pressed', () => {
    ...
    const { getByTestId, getByText } = render(<BankLinkingScreen {...baseProps as any} />);
